...
def info_perfil_session():
    try:
        id_empleado = session.get('id_empleado')  # O la clave que uses

        if not id_empleado:
            print("info_perfil_session: no hay id_empleado en sesión")
            return []
        ...
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                cursor.execute("""
                    SELECT 
                        id_empleado,
                        nombre_empleado AS name_surname,
                        email_empleado  AS email_user
                    FROM tbl_empleados
                    WHERE id_empleado = %s
                    LIMIT 1
                """, (id_empleado,))
